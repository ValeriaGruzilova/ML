...
def compute_hypothesis(X, theta):
    ...
    ones = np.reshape([1] * X.shape[0], (-1, 1))
    X_fict = np.concatenate((ones, X), axis=1)

    Z2 = X_fict @ Theta1.T  # Будущая матрица входов скрытого слоя
    A2 = logistic(X_fict @ Theta1.T)  # Будущая матрица выходов скрытого слоя

    ones1 = np.reshape([1] * A2.shape[0], (-1, 1))
    A2_fict = np.concatenate((ones1, A2), axis=1)
...
def compute_cost_grad(X, y, theta, lamb):
    ...
    # Delta_1 = np.zeros((HIDDEN_LAYER_SIZE, X.shape[1] + 1))
    # Delta_2 = np.zeros((10, HIDDEN_LAYER_SIZE + 1))

    Delta_1 = (X_fict.T @ delta2).T
    Delta_2 = (A2_fict.T @ delta3).T
    Theta1[:, 0] = [0] * HIDDEN_LAYER_SIZE
    Theta2[:, 0] = [0] * 10
